 ...
 */
    /*try {
        ...
        const updatedPokemon = await Pokemon.findOneAndUpdate({ id : pokemonId }, updateData, { new: true });
    ...
    try {
        const updatedPokemon = yield pokemons_1.default.findOneAndUpdate({ id: req.params.id }, req.body, { new: true });
        if (!updatedPokemon)
            return res.status(404).json({ message: "Pokemon non trouvé" });
        res.status(200).json(updatedPokemon);
    }
    catch (error) {
        res.status(400).json({ message: 'Une erreur est survenue lors de la mise à jour du Pokémon.',
            error: error.message });
    }
}));
